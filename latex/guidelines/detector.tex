...
\SI{4}{\tesla\metre}, and three stations of silicon-strip detectors and straw
...
a relative uncertainty that varies from \SI{0.5}{\percent} at low momentum to \SI{1}{\percent} at \SI{200}{\GeVc}.
The minimum distance of a track to a primary vertex (PV), the impact parameter (IP), 
is measured with a resolution of $(15+29/\pt)\,\si{\micro\metre}$,
where \pt is the component of the momentum transverse to the beam, in \si{\GeVc}.
...
with $\pt>500\,(300)\,\si{\MeVc}$ are reconstructed for 2011\,(2012) data.
...
\item At the hardware trigger stage, events are required to have a muon with high \pt or a
  hadron, photon or electron with high transverse energy in the calorimeters. For hadrons,
	the transverse energy threshold is \SI{3.5}{\GeV}.
  ...
  $pp$ interaction vertex. At least one charged particle
	must have a transverse momentum $\pt > \SI{1.6}{\GeVc}$ and be
...
\item The $\decay{\Bd}{\Kstarz\mumu}$ signal candidates are first required
      to pass the hardware trigger, which selects events containing at least
			one muon with transverse momentum $\pt>\SI{1.48}{\GeVc}$ in the \SI{7}{\TeV} data or
	  $\pt>\SI{1.76}{\GeVc}$ in the \SI{8}{\TeV} data.  In the subsequent software
      trigger, at least one of the final-state particles is required to 
	  have $\pt>\SI{1.7}{\GeVc}$ in the \SI{7}{\TeV} data or $\pt>\SI{1.6}{\GeVc}$ in the \SI{8}{\TeV} 
      data, unless the particle is identified as a muon in which case 
	  $\pt>\SI{1.0}{\GeVc}$ is required. The final-state particles that 
      satisfy these transverse momentum criteria are also required 
	  to have an impact parameter larger than $\SI{100}{\micro\metre}$ with respect 
...
%  in the \SI{7}{\TeV} data or $\pt>1.76\gevc$ in the \SI{8}{\TeV} data.
...
are implemented using the \geant toolkit~\cite{Geant} as described in Ref.~\cite{LHCb-PROC-2011-006}.
\end{itemize}
